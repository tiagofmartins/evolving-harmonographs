...
void setup() {
  ...
  grid = calculateGrid(pop.getSize(), 0, 0, width, height, 30, 10, 10, false);
}

void draw() {
  background(255);
  noFill();
  strokeWeight(grid[0][0].z * 0.0025);
  stroke(0);
  int row = 0, col = 0;
  for (int i = 0; i < pop.getSize(); i++) {
    stroke(colors[i]);
    pop.getIndiv(i).render(getGraphics(), grid[row][col].x + grid[row][col].z / 2, grid[row][col].y + grid[row][col].z / 2, grid[row][col].z, grid[row][col].z);
    col += 1;
