...
class Population {
  ...
  void initialize() {
    ...
    float[] fitness_values = evaluator.calculateFitness(individuals);
    for (int i = 0; i < individuals.length; i++) {
      individuals[i].setFitness(fitness_values[i]);
    }
  ...
  void evolve() {
    ...
    float[] fitness_values = evaluator.calculateFitness(new_generation);
    for (int i = 0; i < new_generation.length; i++) {
      new_generation[i].setFitness(fitness_values[i]);
    }
